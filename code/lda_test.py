...
def lda(n_words=15, n_topics=200):
    ...
    if not 'resume_new_data.p' in os.listdir('../careerPaths/test/'):
    	build_dataset()
    	sequence_data = p.load(open('../careerPaths/test/resume_new_data.p', 'rb'))
    else:
    	sequence_data = p.load(open('../careerPaths/test/resume_new_data.p', 'rb'))
    ...
    tf_vectorizer = p.load(open('../careerPaths/models/tf_vect.p','rb'))
    ...
    lda_model = p.load(open('../careerPaths/models/lda_200_topics.p'))

    start_time = timeit.default_timer()

    # Fit model to data
    print '...Fitting LDA model to job description text.\n'
    fitted_data = lda_model.transform(tf)
    ...
    p.dump([ [ fitted_data[idx] for idx in xrange(job_count) ] for job_count in job_sequence_counts ], open('../careerPaths/data/fitted_sequential_data_test' + str(n_topics) + '.p', 'wb'))
